...
def access_polygon(tickers_df, output, from_, to, time_span):
    ...
    for record, row in tickers_df.iterrows():
        ...
        if record_count > 4: 
            ...
            time.sleep(60) 
            print (ticker)
            get_stock_price(stock_key, output, ticker, time_span, from_, to)    
...
######################################################################################
def get_stock_price(client_key, output, ticker, time_span, from_, to):
...
if __name__ == '__main__':
    ...
    access_polygon(key, tickers_df, output, from_, to, time_span)