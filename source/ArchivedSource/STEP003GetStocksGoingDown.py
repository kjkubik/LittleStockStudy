...
data = pd.read_csv('resources/HistoricalData/StockResponseTEST.csv', parse_dates=['date'])
...
def check_price_drop(data, days=7):
    count_stocks_moved = 0
    # Group data by ticker
    grouped = data.groupby('ticker')

    # Open the file to write once, using 'w' for write mode
    with open('resources/HistoricalData/SevenDayLow.csv', 'w') as f:
        ...
        for ticker, ticker_data in grouped:
            ...
            for i in range(days - 1, len(ticker_data)):
                ...
                if (current_price < price_7_days_ago) and (ratio > .85 and ratio < .90):
                            ...
                            print(f"All records for {ticker} written to 'SevenDayLow.csv' because it decreased in price over the last 7 days.")
